...
let rec isValType (ctx : ctx) (t : value_type) : (unit, string) result =
  ...
  | Tensor ts -> (
      ...
      match Base.Result.all tys with Ok _ -> Ok () | Error e -> Error e)
  | Sum ts -> (
      let tys = List.map (isValType ctx) ts in
      match Base.Result.all tys with Ok _ -> Ok () | Error e -> Error e)
  | Tvar t -> (
...
let rec infer_value_type (ctx : ctx) (v : value_term) :
...
and infer_comp_type (ctx : ctx) (c : comp_term) : (comp_type, string) result =
  match c with
  | Ret v -> (
      match infer_value_type ctx v with Ok t -> Ok (F t) | Error e -> Error e)
  | Bind (c1, x, c2) -> (
      ...
      | Ok ct -> (
          ...
          | F a ->
              infer_comp_type
                { ctx with valCtx = Context.add x a ctx.valCtx }
                c2
          | _ ->
              Error
                (Printf.sprintf
