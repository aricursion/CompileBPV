...
let rec infer_tc_val (ctx : value_type Context.t) (v : value_term) :
    (value_type, string) result =
  match v with
  | Var x -> (
      ...
      | _ ->
          Error
            (Printf.sprintf "Injection is not annotated with sum type: %s"
               (pp_term (Val v))))
  | Susp c -> (
      match infer_tc_comp ctx c with Ok ct -> Ok (U ct) | Error e -> Error e)

and infer_tc_comp (ctx : value_type Context.t) (c : comp_term) :
    (comp_type, string) result =
  match c with
  | Ret v -> (
      match infer_tc_val ctx v with Ok t -> Ok (F t) | Error e -> Error e)
  | Bind (c1, x, c2) -> (
      ...
      | Ok ct -> (
          ...
          | _ ->
              Error
                (Printf.sprintf
                   "First computation in Bind does not have F type: %s"
                   (pp_term (Comp c1))))
      | Error e -> Error e)
  | Lam (x, typ, c') -> (
  ...
  | Ap (c1, v2) -> (
      match infer_tc_comp ctx c1 with
      | Ok (Arr (t1, t2)) -> (
          match tc_val ctx v2 t1 with Ok _ -> Ok t2 | Error e -> Error e)
      | Ok t ->
          Error
            (Printf.sprintf
  ...
  | Case (v, arms) -> (
      ...
      | Ok (Sum ts) -> (
          ...
          | Ok ts' ->
              ...
              else if List.for_all (fun t -> t = List.nth ts' 0) ts' then
                Ok (List.nth ts' 0)
              else
                Error
...
and tc_val (ctx : value_type Context.t) (v : value_term) (t : value_type) :
    (unit, string) result =
  match infer_tc_val ctx v with
  | Ok t' ->
      if t = t' then Ok ()
        ...
        Error
          (Printf.sprintf "Expected %s to have type %s, got %s"
             (pp_term (Val v)) (pp_typ (ValTyp t)) (pp_typ (ValTyp t')))
  | Error e -> Error e

and tc_comp (ctx : value_type Context.t) (c : comp_term) (t : comp_type) :
    (unit, string) result =
  ...
  | Ok t' ->
      if t = t' then Ok ()
        ...
        Error
          (Printf.sprintf "Expected %s to have type %s, got %s"
             (pp_term (Comp c)) (pp_typ (CompTyp t)) (pp_typ (CompTyp t')))
  | Error e -> Error e
